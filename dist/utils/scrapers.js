...
export const teamDetailsScraper = (html) => {
    ...
    const recordHeader = document
        ...
        ?.textContent?.split("-")
        .map((r) => r.trim());
    const coach = document.querySelector("ul.d3-o-list > li div.nfl-c-team-info__info-value")?.textContent;
    const stadium = document.querySelector("ul.d3-o-list > li + li div.nfl-c-team-info__info-value")?.textContent;
...
export const teamRosterScraper = (html) => {
    ...
    tableRowEls.forEach((tr) => {
        ...
        const experience = tr.querySelector("td + td + td + td + td + td + td")?.textContent;
        const college = tr.querySelector("td + td + td + td + td + td + td + td")?.textContent;
        roster.push({
            headshot,
            name,
            playerUrl,
...
export const playerScraper = (html) => {
    const document = getDocument(html);
    const status = document.querySelector("h3.nfl-c-player-header__roster-status")
